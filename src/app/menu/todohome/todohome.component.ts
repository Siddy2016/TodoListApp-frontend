...
export class TodohomeComponent implements OnInit {
  ...
  public newtodo = () => {
    if (!this.name) {
      ...
      this.appService.createTodo(values).subscribe((apiResponse: any) => {
  ...
  public deletetodoList = (id) => {
    this.appService.removetodolist(id).subscribe((apiResponse: any) => {
  ...
  public getAlltodoList = () => {
    ...
    this.appService.getAlltodo().subscribe((apiResponse: any) => {
  ...
  public createtask = () => {
    ...
    if (this.task == undefined || this.task == "" || this.task == null) {
      ...
      this.appService.createnewtask(data).subscribe((apiResponse: any) => {
        this.task = "";
        this.getAlltasks();
  ...
  public deletetask = (id) => {
    this.appService.deletesingletask(id).subscribe((apiResponse: any) => {
      if (apiResponse.status == 200) {
        this.toastr.success(apiResponse.message)
        this.getAlltasks();
      }
